# Define the OU and the filter for Windows servers
$ou = "OU=Servers,DC=domain,DC=local"

# Calculate the date for 3 months ago
$threeMonthsAgo = (Get-Date).AddMonths(-3)
$lastLogonTimestampThreshold = [DateTime]::UtcNow.AddMonths(-3).ToFileTime()

# Convert to the lastLogonTimestamp format
$lastLogonTimestampFilter = "(&(objectCategory=computer)(operatingSystem=*Windows*)(lastLogonTimestamp>=$lastLogonTimestampThreshold))"

# Get the list of servers
$servers = Get-ADComputer -SearchBase $ou -LDAPFilter $lastLogonTimestampFilter | Select-Object -ExpandProperty Name
