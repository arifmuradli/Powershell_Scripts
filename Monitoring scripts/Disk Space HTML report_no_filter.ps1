...
$serversNotInContactCount = $serversNotInContact.Count
...
<body>
    ...
    <p>Servers not in contact ($serversNotInContactCount):</p>
    <ul>
"@

# Add the list of servers not in contact to the HTML content
foreach ($server in $serversNotInContact) {
    $htmlContent += "<li>$server</li>"
}

# Continue the HTML content
$htmlContent += @"
    </ul>
    <table id="diskTable">
        <tr>
